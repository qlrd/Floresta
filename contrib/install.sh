...
OPTIONS=$(getopt -o x:d:n:t:p:C:v:uscUNh --long xpub:,desc:,network:,tag:,proxy:,connect:,assume-valid:,assume-utreexo,ssl,cfilters,uninstall,non-interactive,help -n "$0" -- "$@")
if [ $? -ne 0 ]; then
  show_usage
  exit 1
fi
...
interactive_review() {
  ...
  review_message+="Assume Valid:       $assume_valid\n"
