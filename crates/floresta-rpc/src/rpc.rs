...
pub trait FlorestaRPC {
    ...
    /// and the nonce.
    fn get_block_header(&self, hash: BlockHash, verbose: Option<bool>) -> Result<BlockHeader>;
...
impl<T: JsonRPCClient> FlorestaRPC for T {
    ...
    fn get_block_header(&self, hash: BlockHash, verbose: Option<bool>) -> Result<BlockHeader> {
        let verbosity = if let Some(v) = verbose {
            Value::Bool(v)
        } else {
            Value::Bool(false)
        };
        ...
        self.call(
            "getblockheader",
            &[Value::String(hash.to_string()), verbosity],
        )
    }
